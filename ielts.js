(function() { 
		...
		$.ajax({
			...
			success: function(data) {
				...
				page.find("#months").find("input[type='checkbox']").each(function() {
					var el = $(this);
					var date_value = trim(el.val());
					var date_name = trim(el.parent().text());
  ...
  function query_test_date() {
    console.log("query test date");
    $("#process-3-date").show();
  }

	function test_start(){
		var date = [];
		var date_select = $("#date").val();
		for (var i = 0; i < date_select.length; i++) {
			var date_value = date_select[i];
			var date_name = $('#date').find('option[value="' + date_value + '"]').text();
			date.push({
		...
		var city = $("#city").val();
		for (var u = 0; u < city.length; u++) {
			var city_value = city[u];
			var city_name = $('#city').find('option[value="' + city_value + '"]').text();
	...
	function test_load(){
		...
		for (var s = 0; s < select.length; s++) {
			var city_value = select[s].city_value;
			var city_name = select[s].city_name;
			var date_value = select[s].date_value;
			var date_name = select[s].date_name;
